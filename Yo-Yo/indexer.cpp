...
void TIndexer::IndexText(const std::string& text) {
	...
	int next_id = sentence_index_.size();
	if(sentence_index_.find(sentence) != sentence_index_.end()) continue;
	...
	sentences_.push_back(sentence);
    	...
    	for (const auto& word : words) IndexSentence(word, sentence, sentences_.size());
    }
}
